...
## 🚀 特徴 (Features)
  ...
  * **CI/CD対応**: GitHub Actionsなどのビルドパイプラインで、機密情報を安全に扱うための自動生成機能（環境変数対応）を標準搭載しています。
...
1. GitHubリポジトリの `Settings > Secrets and variables > Actions` に、以下の3つのSecretを登録します。
   ...
   * `UNIGAS_AUTH_TOKEN`
   * `UNIGAS_SHEET_ID`
2. GitHub Actionsのワークフロー（`.yml`）のUnityビルドステップに、`env` としてSecretを渡します。

**GitHub Actions (.yml) の設定例:**
```yaml
      - name: Build Unity Project
        uses: game-ci/unity-builder@v4
        env: # ここで環境変数を渡すだけで、ビルド前に自動でアセットが生成されます
          UNIGAS_DEPLOY_ID: ${{ secrets.UNIGAS_DEPLOY_ID }}
          UNIGAS_AUTH_TOKEN: ${{ secrets.UNIGAS_AUTH_TOKEN }}
          UNIGAS_SHEET_ID: ${{ secrets.UNIGAS_SHEET_ID }}
        with:
